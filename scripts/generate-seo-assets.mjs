...
const SITE_URL = (process.env.PUBLIC_SITE_URL ?? 'https://www.cloudcertprep.io').replace(/\/$/, '')
const APEX_HOST = new URL(SITE_URL).hostname.replace(/^www\./, '')
const WWW_HOST = `www.${APEX_HOST}`
const isUnofficialFork = process.env.PUBLIC_UNOFFICIAL_FORK === 'true'
...
const hostRedirect = isUnofficialFork
  ? `https://${WWW_HOST}/*   https://${APEX_HOST}/:splat   301!`
...
# Primary host redirect (permanent)
${hostRedirect}
...
manifest.name = isUnofficialFork
  ? 'Unofficial CloudCertPrep Learning Mirror'
  : 'CloudCertPrep · Free Cloud Certification Practice Exams'
manifest.description = isUnofficialFork
  ? 'An unofficial, noindex learning mirror for browser translation. Source: CloudCertPrep under the MIT License.'
  : `Free cloud certification practice exams across ${activeCerts.length} active ${activeCerts.length === 1 ? 'certification' : 'certifications'} (${totalActive.toLocaleString('en-GB')}+ questions). Timed mock exams, domain practice with adaptive spaced repetition, and progress tracking. No signup, no ads.`
